...
export const updateWorkoutEntry = async (userId, entryId, workoutData) => {
  try {
    ...
    const updateFields = {};
    if (workoutData.date)
      updateFields["progress.workoutHistory.$.date"] = new Date(
        workoutData.date
      );
    if (workoutData.workoutId !== undefined)
      updateFields["progress.workoutHistory.$.workoutId"] =
        workoutData.workoutId;
    if (workoutData.notes !== undefined)
      updateFields["progress.workoutHistory.$.notes"] = workoutData.notes;
